...
Here are some suggestions and best practices for obtaining data in React and Next.js. This section will introduce:

###5.1 Try to obtain data from the server as much as possible

Getting data from the server as much as possible has many benefits, such as:

1. Can directly access backend resources (such as databases)
2. Prevent sensitive information leakage
3. Reduce round-trip communication between the client and server, and accelerate response time
   4

###5.2 Obtain data locally where needed

If multiple components in the component tree use the same data, there is no need to globally retrieve it first and then pass it through props. You can directly use 'fetch' or React 'cache' to retrieve data where you need it, without worrying about performance issues caused by multiple requests, as' fetch 'requests will be automatically memorized. This also applies to layouts, as data cannot be transmitted between parent-child layouts.

###5.3 Use Streaming when appropriate

Streaming and Suspend are both React features that allow you to incrementally transfer content and gradually render UI units. The page can directly render part of the content, and the remaining part that retrieves data will display loading status, which also means that users do not need to wait until the page is fully loaded before interacting with it.

![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e0271e01ad344ceeaa4059be34513311~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=1600&h=785&s=475591&e=png&b=151515)

###5.4 Serial data acquisition

There are two data acquisition modes when retrieving data within the React component, parallel and serial.

![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/1731c628568e4746957939c94929c309~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=1600&h=525&s=338982&e=png&b=141414)

The so-called serial data acquisition refers to the interdependence of data requests, forming a waterfall structure. This behavior is sometimes necessary, but it can also lead to longer loading times.

The so-called parallel data acquisition, where requests occur simultaneously and data is loaded, reduces the total time required to load data.

Let's first talk about serial data acquisition, for example:

<pre><div class="code-block-extension-header"><div class="code-block-extension-headerLeft"><div class="code-block-extension-foldBtn"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M16.924 9.617A1 1 0 0 0 16 9H8a1 1 0 0 0-.707 1.707l4 4a1 1 0 0 0 1.414 0l4-4a1 1 0 0 0 .217-1.09z" data-name="Down"></path></svg></div></div><div class="code-block-extension-headerRight"></div></div><code class="hljs language-javascript code-block-extension-codeShowNum"><span class="code-block-extension-codeLine" data-line-num="1">// app/artist/page.js</span>
<span class="code-block-extension-codeLine" data-line-num="2">// ...</span>
<span class="code-block-extension-codeLine" data-line-num="3"></span>
<span class="code-block-extension-codeLine" data-line-num="4">async function Playlists({ artistID }) {</span>
<span class="code-block-extension-codeLine" data-line-num="5">  // Waiting for Playlists data</span>
<span class="code-block-extension-codeLine" data-line-num="6">  const playlists = await getArtistPlaylists(artistID)</span>
<span class="code-block-extension-codeLine" data-line-num="7"></span>
<span class="code-block-extension-codeLine" data-line-num="8">  return (</span>
<span class="code-block-extension-codeLine" data-line-num="9">    <span class="xml">&lt;ul&gt;</span></span>
<span class="code-block-extension-codeLine" data-line-num="10">      {playlists.map((playlist) =&gt; (</span>
<span class="code-block-extension-codeLine" data-line-num="11">        &lt;li key={playlist.id}&gt;{playlist.name}&lt;/li&gt;</span>
<span class="code-block-extension-codeLine" data-line-num="12">      ))}</span>
<span class="code-block-extension-codeLine" data-line-num="13">    &lt;/ul&gt;</span>
<span class="code-block-extension-codeLine" data-line-num="14">  )</span>
<span class="code-block-extension-codeLine" data-line-num="15">}</span>
<span class="code-block-extension-codeLine" data-line-num="16"></span>
<span class="code-block-extension-codeLine" data-line-num="17">export default async function Page({ params: { username } }) {</span>
<span class="code-block-extension-codeLine" data-line-num="18">  // Waiting for artist data</span>
<span class="code-block-extension-codeLine" data-line-num="19">  const artist = await getArtist(username)</span>
<span class="code-block-extension-codeLine" data-line-num="20"></span>
<span class="code-block-extension-codeLine" data-line-num="21">  return (</span>
<span class="code-block-extension-codeLine" data-line-num="22">    <span class="xml">&lt;&gt;</span></span>
<span class="code-block-extension-codeLine" data-line-num="23">      &lt;h1&gt;{artist.name}&lt;/h1&gt;</span>
<span class="code-block-extension-codeLine" data-line-num="24">      &lt;Suspense fallback={&lt;div&gt;Loading...&lt;/div&gt;}&gt;</span>
<span class="code-block-extension-codeLine" data-line-num="25">        &lt;Playlists artistID={artist.id} /&gt;</span>
<span class="code-block-extension-codeLine" data-line-num="26">      &lt;/Suspense&gt;</span>
<span class="code-block-extension-codeLine" data-line-num="27">    &lt;/&gt;</span>
<span class="code-block-extension-codeLine" data-line-num="28">  )</span>
<span class="code-block-extension-codeLine" data-line-num="29">}</span>
</code></pre>

In this example, the 'Playlists' component only starts retrieving data when the' Artist 'component obtains it, because the' Playlists' component relies on the 'artistId' prop. This is also easy to understand, after all, only by knowing which artist it is can we obtain the corresponding repertoire for that artist.

In this case, you can use the 'loading. js' or React's'<Suspend>' component to display an instant loading status, preventing the entire route from being blocked by data requests, and users can also interact with the unblocked parts.

Regarding blocking data requests:

*One way to prevent serial data requests is to globally retrieve data at the root of the application, but this will block the rendering of all routing segments below it until the data is loaded.
*Any fetch request that uses' await 'will block data requests for rendering and all components below, unless they use'<Suspend>'or' loading. js'. Another alternative is to use parallel data requests or preloading mode.

###5.5 Parallel data request

To achieve parallel data request, you can define the request outside the component that uses the data, and then call it inside the component. For example:

<pre><div class="code-block-extension-header"><div class="code-block-extension-headerLeft"><div class="code-block-extension-foldBtn"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M16.924 9.617A1 1 0 0 0 16 9H8a1 1 0 0 0-.707 1.707l4 4a1 1 0 0 0 1.414 0l4-4a1 1 0 0 0 .217-1.09z" data-name="Down"></path></svg></div></div><div class="code-block-extension-headerRight"></div></div><code class="hljs language-javascript code-block-extension-codeShowNum"><span class="code-block-extension-codeLine" data-line-num="1">import Albums from &#39;./albums&#39;</span>
<span class="code-block-extension-codeLine" data-line-num="2"></span>
<span class="code-block-extension-codeLine" data-line-num="3">// Component External Definition</span>
<span class="code-block-extension-codeLine" data-line-num="4">async function getArtist(username) {</span>
<span class="code-block-extension-codeLine" data-line-num="5">  const res = await fetch(`https://api.example.com/artist/${username}`)</span>
<span class="code-block-extension-codeLine" data-line-num="6">  return res.json()</span>
<span class="code-block-extension-codeLine" data-line-num="7">}</span>
<span class="code-block-extension-codeLine" data-line-num="8"></span>
<span class="code-block-extension-codeLine" data-line-num="9">async function getArtistAlbums(username) {</span>
<span class="code-block-extension-codeLine" data-line-num="10">  const res = await fetch(`https://api.example.com/artist/${username}/albums`)</span>
<span class="code-block-extension-codeLine" data-line-num="11">  return res.json()</span>
<span class="code-block-extension-codeLine" data-line-num="12">}</span>
<span class="code-block-extension-codeLine" data-line-num="13"></span>
<span class="code-block-extension-codeLine" data-line-num="14">export default async function Page({ params: { username } }) {</span>
<span class="code-block-extension-codeLine" data-line-num="15">  // Invoke within the component, which is parallel here</span>
<span class="code-block-extension-codeLine" data-line-num="16">  const artistData = getArtist(username)</span>
<span class="code-block-extension-codeLine" data-line-num="17">  const albumsData = getArtistAlbums(username)</span>
<span class="code-block-extension-codeLine" data-line-num="18"></span>
<span class="code-block-extension-codeLine" data-line-num="19">  // wait promise resolve</span>
<span class="code-block-extension-codeLine" data-line-num="20">  const [artist, albums] = await Promise.all([artistData, albumsData])</span>
<span class="code-block-extension-codeLine" data-line-num="21"></span>
<span class="code-block-extension-codeLine" data-line-num="22">  return (</span>
<span class="code-block-extension-codeLine" data-line-num="23">    <span class="xml">&lt;&gt;</span></span>
<span class="code-block-extension-codeLine" data-line-num="24">      &lt;h1&gt;{artist.name}&lt;/h1&gt;</span>
<span class="code-block-extension-codeLine" data-line-num="25">      &lt;Albums list={albums}&gt;&lt;/Albums&gt;</span>
<span class="code-block-extension-codeLine" data-line-num="26">    &lt;/&gt;</span>
<span class="code-block-extension-codeLine" data-line-num="27">  )</span>
<span class="code-block-extension-codeLine" data-line-num="28">}</span>
</code></pre>

In this example, both the 'getArtist' and 'getArtistAlbums' functions are defined outside the' Page 'component and then called inside the' Page 'component. Users need to wait for both promises to be resolved before they can see the results.

To improve user experience, the Suspend component can be used to decompose rendering work and display partial results as soon as possible.

###5.6 Preload data

Another way to prevent serial requests is to use preloading. For example:

<pre><div class="code-block-extension-header"><div class="code-block-extension-headerLeft"><div class="code-block-extension-foldBtn"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M16.924 9.617A1 1 0 0 0 16 9H8a1 1 0 0 0-.707 1.707l4 4a1 1 0 0 0 1.414 0l4-4a1 1 0 0 0 .217-1.09z" data-name="Down"></path></svg></div></div><div class="code-block-extension-headerRight"></div></div><code class="hljs language-javascript code-block-extension-codeShowNum"><span class="code-block-extension-codeLine" data-line-num="1">// app/article/[id]/page.js</span>
<span class="code-block-extension-codeLine" data-line-num="2">import Article, { preload, checkIsAvailable } from &#39;./components/Article&#39;</span>
<span class="code-block-extension-codeLine" data-line-num="3"></span>
<span class="code-block-extension-codeLine" data-line-num="4">export default async function Page({ params: { id } }) {</span>
<span class="code-block-extension-codeLine" data-line-num="5">  // Get article data</span>
<span class="code-block-extension-codeLine" data-line-num="6">  preload(id)</span>
<span class="code-block-extension-codeLine" data-line-num="7">  // Execute another asynchronous task, here is pseudocode, such as checking if the article has access permissions</span>
<span class="code-block-extension-codeLine" data-line-num="8">  const isAvailable = await checkIsAvailable()</span>
<span class="code-block-extension-codeLine" data-line-num="9"></span>
<span class="code-block-extension-codeLine" data-line-num="10">  return isAvailable ? <span class="xml">&lt;Article id={id} /&gt;</span> : null</span>
<span class="code-block-extension-codeLine" data-line-num="11">}</span>
</code></pre>

In the specific preload function, it should be used in conjunction with the React cache function:

<pre><div class="code-block-extension-header"><div class="code-block-extension-headerLeft"><div class="code-block-extension-foldBtn"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M16.924 9.617A1 1 0 0 0 16 9H8a1 1 0 0 0-.707 1.707l4 4a1 1 0 0 0 1.414 0l4-4a1 1 0 0 0 .217-1.09z" data-name="Down"></path></svg></div></div><div class="code-block-extension-headerRight"></div></div><code class="hljs language-javascript code-block-extension-codeShowNum"><span class="code-block-extension-codeLine" data-line-num="1">// components/Article.js</span>
<span class="code-block-extension-codeLine" data-line-num="2">import { getArticle } from &#39;@/utils/get-article&#39;</span>
<span class="code-block-extension-codeLine" data-line-num="3">import { cache } from &#39;react&#39;</span>
<span class="code-block-extension-codeLine" data-line-num="4"></span>
<span class="code-block-extension-codeLine" data-line-num="5">export const getArticle = cache(async (id) =&gt; {</span>
<span class="code-block-extension-codeLine" data-line-num="6">  // ...</span>
<span class="code-block-extension-codeLine" data-line-num="7">})</span>
<span class="code-block-extension-codeLine" data-line-num="8"></span>
<span class="code-block-extension-codeLine" data-line-num="9">export const preload = (id) =&gt; {</span>
<span class="code-block-extension-codeLine" data-line-num="10">	void getArticle(id)</span>
<span class="code-block-extension-codeLine" data-line-num="11">}</span>
<span class="code-block-extension-codeLine" data-line-num="12"></span>
<span class="code-block-extension-codeLine" data-line-num="13">export const checkIsAvailable = (id) =&gt; {</span>
<span class="code-block-extension-codeLine" data-line-num="14">	// ...</span>
<span class="code-block-extension-codeLine" data-line-num="15">}</span>
<span class="code-block-extension-codeLine" data-line-num="16"></span>
<span class="code-block-extension-codeLine" data-line-num="17">export default async function Article({ id }) {</span>
<span class="code-block-extension-codeLine" data-line-num="18">  const result = await getArticle(id)</span>
<span class="code-block-extension-codeLine" data-line-num="19">  // ...</span>
<span class="code-block-extension-codeLine" data-line-num="20">}</span>
</code></pre>

###5.7 Use React `cache` server only ` and preload mode

You can combine the 'cache' function, 'preload' mode, and [server only](https://juejin.cn/book/7307859898316881957/section/7309076661532622885#heading-15 "https://juejin.cn/book/7307859898316881957/section/7309076661532622885#heading-15") Use the package together to create a data request tool function that can be used throughout the entire application.

<pre><div class="code-block-extension-header"><div class="code-block-extension-headerLeft"><div class="code-block-extension-foldBtn"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M16.924 9.617A1 1 0 0 0 16 9H8a1 1 0 0 0-.707 1.707l4 4a1 1 0 0 0 1.414 0l4-4a1 1 0 0 0 .217-1.09z" data-name="Down"></path></svg></div></div><div class="code-block-extension-headerRight"></div></div><code class="hljs language-javascript code-block-extension-codeShowNum"><span class="code-block-extension-codeLine" data-line-num="1">// utils/get-article.js</span>
<span class="code-block-extension-codeLine" data-line-num="2">import { cache } from &#39;react&#39;</span>
<span class="code-block-extension-codeLine" data-line-num="3">import &#39;server-only&#39;</span>
<span class="code-block-extension-codeLine" data-line-num="4"></span>
<span class="code-block-extension-codeLine" data-line-num="5">export const preloadArticle = (id) =&gt; {</span>
<span class="code-block-extension-codeLine" data-line-num="6">  void getArticle(id)</span>
<span class="code-block-extension-codeLine" data-line-num="7">}</span>
<span class="code-block-extension-codeLine" data-line-num="8"></span>
<span class="code-block-extension-codeLine" data-line-num="9">export const getArticle = cache(async (id) =&gt; {</span>
<span class="code-block-extension-codeLine" data-line-num="10">  // ...</span>
<span class="code-block-extension-codeLine" data-line-num="11">})</span>
</code></pre>

Now, you can retrieve data in advance, cache the returned results, and ensure that data retrieval only occurs on the server side. In addition, layout, pages, and components can all use ` utils/get article. js`
